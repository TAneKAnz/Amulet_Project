...
# Function to create a new folder with an incremented number if it already exists
def create_augmented_folder(base_folder_path):
    counter = 0
    base_folder_path = os.path.join(os.path.dirname(base_folder_path), f'Aug{os.path.basename(base_folder_path)}')
    folder_path = base_folder_path

    while os.path.exists(folder_path):
        counter += 1
        folder_path = f"{base_folder_path}{counter}"
        print(f"{base_folder_path}{counter}")
...
def augment_images_for_all_folders(main_folder, output_folder_prefix='Aug', num_augmentations=5):
    ...
    #augcepp_output_folder = os.path.join(os.path.dirname(main_folder), f'Aug{os.path.basename(main_folder)}')  # Updated output folder path
...
# Perform image augmentation for all folders in the specified directory
#create_augmented_folder(main_folder)
