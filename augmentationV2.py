...
def augment_images_for_all_folders(main_folder, output_folder_prefix='Aug', num_augmentations=5):
    ...
    # Check if 'AugCEPP' folder already exists
    if 'AugCEPP' not in folders:
        highest_number = 0
    else:
        # Find the highest numbered existing 'AugCEPP' folder
        existing_augcepp_folders = [folder for folder in folders if folder.startswith('AugCEPP')]
        existing_augcepp_folders.sort(reverse=True)

        if existing_augcepp_folders:
            highest_number = int(existing_augcepp_folders[0].replace('AugCEPP', ''))
        else:
            highest_number = 0

    # Construct the new 'AugCEPP' folder name
    new_number = highest_number + 1
    augcepp_output_folder = os.path.join(os.path.dirname(main_folder), f'AugCEPP{new_number}')
